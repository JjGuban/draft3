...
function getPurchase($user_id, $manga_id) {
    global $conn;
    $stmt = $conn->prepare("SELECT * FROM purchases WHERE user_id = ? AND manga_id = ?");
    $stmt->execute([$user_id, $manga_id]);
    return $stmt->fetch();
}
...
function addPurchase($user_id, $manga_id) {
    global $conn;
    $stmt = $conn->prepare("INSERT INTO purchases (user_id, manga_id, purchase_date) VALUES (?, ?, NOW())");
    $stmt->execute([$user_id, $manga_id]);
}
...
function cancelPurchase($purchase_id, $user_id) {
    global $conn;
    $stmt = $conn->prepare("UPDATE purchases SET cancel_date = NOW() WHERE purchase_id = ? AND user_id = ?");
...
function getActivePurchases($user_id) {
    global $conn;
    $stmt = $conn->prepare("SELECT * FROM purchases WHERE user_id = ? AND cancel_date IS NULL");
    $stmt->execute([$user_id]);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
function getCanceledPurchases($user_id) {
    global $conn;
    $stmt = $conn->prepare("SELECT * FROM purchases WHERE user_id = ? AND cancel_date IS NOT NULL");
    $stmt->execute([$user_id]);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
